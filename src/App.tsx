...
function RenderedOutput({ document, selectedBlockId, onSelectBlock }: { document: DocumentResult; selectedBlockId: string | null; onSelectBlock: (blockId: string) => void }) {
  ...
  return <article className="rendered-document" aria-label="Rendered conversion preview">
    {document.blocks.map((block) => {
      ...
      const props = { key: block.id, className: selected ? "is-selected" : undefined, onClick: select, tabIndex: 0, role: "button" as const, "aria-label": `Inspect ${block.type} from ${block.page}`, onKeyDown: (event: React.KeyboardEvent) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); select(); } } };
      if (block.type === "heading") { const Tag = `h${Math.min(4, Math.max(1, block.level || 2))}` as "h1" | "h2" | "h3" | "h4"; return <Tag {...props}>{block.text}</Tag>; }
      if (block.type === "table") { const rows = tableRows(block.text); return rows.length ? <div {...props} className={`output-block-table ${selected ? "is-selected" : ""}`}><table><thead><tr>{rows[0].map((cell, index) => <th key={`${cell}-${index}`} scope="col">{cell}</th>)}</tr></thead><tbody>{rows.slice(1).map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div> : <p {...props}>{block.text}</p>; }
      if (block.type === "formula") return <pre {...props} className={`rendered-formula ${selected ? "is-selected" : ""}`}>{block.text}</pre>;
      if (block.type === "caption") return <p {...props} className={`rendered-caption ${selected ? "is-selected" : ""}`}>{block.text}</p>;
