...
type TaskKind = "conversion" | "preflight" | "repair" | "export" | "library" | "models" | "download";
...
function TaskProgressBar({ progress }: { progress: TaskProgress }) {
  ...
  const title = progress.kind === "preflight" ? "Inspecting files" : progress.kind === "repair" ? "Repairing selected block" : progress.kind === "export" ? "Exporting conversion" : progress.kind === "library" ? "Loading local library" : progress.kind === "models" ? "Inspecting local models" : progress.kind === "download" ? "Downloading model pack" : "Converting locally";
  ...
  return (
    <section className="output-panel">
      ...
      <footer className="output-footer">
        ...
        {document && <div className="output-actions">{document.outputs.page_tree && <a className="export-link" href={convertFileSrc(document.outputs.page_tree)} target="_blank" rel="noreferrer">Page tree JSON</a>}<button className="export-button" type="button" onClick={() => navigator.clipboard.writeText(body)}><ClipboardText size={16} /> Copy {format === "Preview" ? "Markdown" : format}</button></div>}
      </footer>
    </section>
  );
}
...
function App() {
  ...
  // A first run opens model setup once. It is the only moment a person is
  // shown what Philon can fetch, and it records that it happened so an
  // ordinary launch goes straight to the workspace.
  useEffect(() => {
    if (preferences.modelSetupSeen) return;
    updatePreferences({ modelSetupSeen: true });
    setView("models");
    void inspectModels();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [preferences.modelSetupSeen]);
  ...
  /**
   * Fetch one approved pack. Philon opens no connection of its own here: the
   * request goes to the engine, which is the only component with a
   * network-capable module, and which verifies every byte before installing it.
   */
  const downloadModel = async (packId: string) => {
    const jobId = await beginTask("download", `Downloading ${packId.replaceAll("-", " ")}`);
    try {
      const response = await invoke<{ status: string; message?: string; models?: { packs: ModelPack[] } }>(
        "fetch_model", { packId, jobId },
      );
      if (response.models?.packs) setModels(response.models.packs);
      if (response.status !== "installed") setError(response.message || `${packId} was not installed.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { finishTask(jobId); }
  };

  /** Remove a pack Philon fetched. One a person installed is never touched. */
  const removeModel = async (packId: string) => {
    const jobId = await beginTask("models", `Removing ${packId.replaceAll("-", " ")}`);
    try {
      const response = await invoke<{ models?: { packs: ModelPack[] } }>("remove_model", { packId });
      if (response.models?.packs) setModels(response.models.packs);
    } catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { finishTask(jobId); }
  };
  ...
  return (
    <main className="app-shell">
      <section className="workspace">
        ...
        {view === "library" && <section className="secondary-workspace"><div className="secondary-heading"><div><span className="eyebrow"><Archive size={14} weight="fill" /> Library</span><h2>Recent local conversions</h2><p>History is stored in Philon’s local SQLite database on this Mac.</p></div><div className="library-actions">{libraryCleanPending ? <><button className="text-button" onClick={() => setLibraryCleanPending(false)} type="button">Cancel</button><button className="danger-button" onClick={() => void cleanLibrary()} type="button">Remove {history.length} job{history.length === 1 ? "" : "s"}</button></> : <button className="secondary-button" onClick={() => setLibraryCleanPending(true)} disabled={!history.length} type="button">Clean</button>}<button className="secondary-button" onClick={() => void loadHistory(true)} type="button"><ArrowClockwise size={16} /> Refresh</button></div></div>{history.length ? <><div className="history-list">{history.map((item) => <button className="history-item" key={item.id} onClick={() => void openHistoricalJob(item)} type="button"><div><strong>{item.documents} document{item.documents === 1 ? "" : "s"}</strong><span>{timestampLabel(item.created_at)} · {item.profile}</span></div><span className={item.warnings ? "history-warning" : "history-good"}>{item.warnings ? `${item.warnings} warnings` : "Verified"}</span></button>)}</div>{history.some((item) => item.warnings) && <div className="review-queue"><span className="eyebrow"><WarningCircle size={14} weight="fill" /> Cross-document review</span><p>{history.reduce((total, item) => total + item.warnings, 0)} warning{history.reduce((total, item) => total + item.warnings, 0) === 1 ? "" : "s"} remain across locally retained conversions. Select a job above to inspect its evidence.</p></div>}</> : <div className="secondary-empty"><Archive size={30} weight="thin" /> No local conversion history yet.</div>}</section>}
        {view === "models" && <section className="secondary-workspace models-workspace"><div className="secondary-heading"><div><span className="eyebrow"><MagicWand size={14} weight="fill" /> Models</span><h2>Local model access</h2><p>Optional models stay off until you enable them. Philon never uploads a model or your document. A pack you ask for is downloaded from its publisher over HTTPS and installed only if every file matches the SHA-256 recorded in Philon&rsquo;s manifest.</p><p className="model-setup-summary">{modelSetupSummary(models)}</p></div><button className="secondary-button" onClick={() => void inspectModels()} type="button"><ArrowClockwise size={16} /> Refresh</button></div>{models.length ? <div className="model-list">{models.map((pack) => { const enabled = preferences.enabledModelIds.includes(pack.id); const canEnable = Boolean(pack.approved && !pack.required && ["ready", "probe-required"].includes(pack.readiness || "")); const status = pack.readiness === "ready" ? "Ready locally" : pack.readiness === "incomplete" ? "Runtime incomplete" : pack.readiness === "probe-required" ? "Manual probe required" : pack.readiness === "blocked" ? "Blocked by policy" : pack.readiness === "not-found" ? "Not found locally" : pack.readiness === "unavailable" ? "Unavailable" : pack.available_locally ? "Available locally" : "Not installed"; const downloadLabel = packDownloadLabel(pack); const canFetch = Boolean(pack.approved && !pack.required && pack.downloadable && !pack.available_locally); return <article className="model-pack" key={pack.id}><ShieldCheck size={20} /><div><strong>{pack.id.replaceAll("-", " ")}</strong><span>{pack.role} · {pack.runtime}</span><small className="model-licence">{pack.license}</small>{downloadLabel && !pack.available_locally && <small>Download {downloadLabel}</small>}{pack.managed && <small>Installed by Philon into its own model store</small>}{pack.diagnostics?.map((diagnostic) => <small key={diagnostic}>{diagnostic}</small>)}</div><div className="model-actions">{canFetch && <button className="model-toggle" onClick={() => void downloadModel(pack.id)} type="button">Download</button>}{pack.managed && <button className="model-toggle" onClick={() => void removeModel(pack.id)} type="button">Remove</button>}{pack.required ? <b>Built in</b> : <button className={`model-toggle ${enabled ? "is-enabled" : ""}`} disabled={!canEnable} onClick={() => updatePreferences({ enabledModelIds: enabled ? preferences.enabledModelIds.filter((id) => id !== pack.id) : [...preferences.enabledModelIds, pack.id] })} type="button">{enabled ? "Enabled" : canEnable ? "Enable" : status}</button>}</div></article>; })}</div> : <div className="secondary-empty"><MagicWand size={30} weight="thin" /> Inspect local model availability.</div>}</section>}
